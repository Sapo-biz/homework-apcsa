...
public class MasterMind {
	...
	/// figure this out. what does it mean?
	...
	{
		reveal = false;
		guesses = new PegArray(4);
		master = new char[4];
	}
	
	/** main method, creates instance of class and calls runIt */
	public static void main(String[] args)
	{
		Mastermind md = new MasterMind();
		md.runIt();
	}
	
	/** calls all the neccesary functions */
	public void runIt()
	{
		printIntroduction();
		generateCode(); // no need to pass, master is a field var
		acceptCode(); // try over and over, ask user for a code
		
		printBoard();
		printGuess();
	}
	
	/** generates the master code that the user will try to decode */
	public void generateCode()
	{
		// still not sure about the master array
		// fill the array
		for(int i=0; i < master.length; i++)
		{
			master[i] = (char)(int)(Math.random()*6 + 65);
		}
	}
	
	/** asks the user for a code, repeats if it does not meet requirements*/
